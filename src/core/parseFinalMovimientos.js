...
export function parseFinalMovimientosFile(rows, sedeName) {
  ...
  for (let r = headerInfo.headerRowIndex + 1; r < rows.length; r++) {
    ...
    // cuentan para el bloque (no tienen fórmula porque no deben incluirse
    // en el cálculo). Se detecta ANTES de toNum() (que convertiría "" en 0,
    // indistinguible de un 0% real) y se descarta la fila completa cuando
    // la columna existe pero viene en blanco.
